...
<div class="container-fluid px-4 mt-4"> 
    <div class="card mb-4 border-start border-success border-4 shadow-sm"> 
        <div class="card-body"> 
            <h2 class="h4 mb-1 fw-bold text-dark">Registrar Vacinação</h2> 
            <p class="text-muted mb-0">Selecione o paciente e informe a vacina aplicada no atendimento.</p> 
        </div> 
    </div> 
                    ...
                    <select name="animal_id" class="form-select @error('animal_id') is-invalid @enderror" required> 
                        ...
                        @foreach($animals as $animal) 
                            <option value="{{ $animal->id }}" {{ old('animal_id', $selectedAnimalId) == $animal->id ? 'selected' : '' }}> 
                                {{ $animal->name }} ({{ $animal->specie->name ?? 'N/A' }}) — Tutor: {{ $animal->tutor->name ?? 'N/A' }} 
                            </option> 
                        @endforeach 
                    </select> 
